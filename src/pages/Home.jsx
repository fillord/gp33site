          ...
          <div className="max-w-3xl animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="bg-yellow-400 text-teal-900 font-bold py-4 px-8 rounded-full shadow-lg hover:bg-yellow-300 hover:scale-105 transition transform flex items-center justify-center"
              >
                <Calendar className="mr-2" size={20} /> {t.btnAppoint}
              </a>
              <Link
                to="/services/registry"
                className="border-2 border-white text-white font-bold py-4 px-8 rounded-full hover:bg-white hover:text-teal-800 transition flex items-center justify-center"
              >
      ...
      <div className="relative z-20 -mt-10 mb-16 container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {t.links.map((item, i) => (
            <div
              key={i}
              className="bg-white rounded-xl shadow-lg p-6 flex items-center hover:-translate-y-1 transition duration-300"
            >
              <div
                className={`${item.color} w-14 h-14 rounded-full flex items-center justify-center text-white mr-4 shadow-md flex-shrink-0`}
              >
                <item.icon size={24} />
              </div>
              <div>
      ...
      <div className="container mx-auto px-4 pb-20">
        ...
        {loadingNews ? (
          ...
          <div className="text-center text-gray-400 py-10">
            {lang === "ru" ? "Новостей пока нет" : "Жаңалықтар әзірге жоқ"}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {latestNews.map((item) => {
              const displayTitle =
                lang === "kz" && item.titleKz ? item.titleKz : item.title;
              const displayText =
                lang === "kz" && item.textKz ? item.textKz : item.text;

              return (
                <div
                  key={item.id}
                  className="bg-white rounded-xl shadow hover:shadow-xl transition group overflow-hidden border border-gray-100 flex flex-col"
                >
                  {/* БЛОК С КАРТИНКОЙ */}
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <Link
                      to="/news"
                      className="mt-auto text-teal-600 font-bold hover:underline inline-flex items-center"
                    >
