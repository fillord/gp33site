...
export default function PersonalReception() {
  ...
  const t = {
    breadcrumb:
      lang === "ru" ? "ГЛАВНАЯ / ЛИЧНЫЙ ПРИЕМ" : "БАСТЫ БЕТ / ЖЕКЕ ҚАБЫЛДАУ",
    title:
      lang === "ru"
    ...
    subtitle:
      lang === "ru"
        ? "Администрация ГП №33 на 2026 год"
        : "№33 ҚЕ әкімшілігі, 2026 жыл",
    ...
    callBtn:
      lang === "ru"
        ? "Позвонить в приемную"
        : "Қабылдау бөлмесіне қоңырау шалу",
  };
  ...
  const schedule = [
    ...
    },
    {
      id: 2,
      role: {
        ru: "Зам. директора по лечебной части",
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          {schedule.map((person) => (
            <div
              key={person.id}
              className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col md:flex-row group hover:shadow-2xl transition duration-300 border border-gray-100"
            >
              {/* Левая часть: Фото и цветная полоса */}
              <div
                className={`md:w-1/3 ${person.color} p-6 flex flex-col items-center justify-center text-white relative overflow-hidden`}
              >
              ...
              <div className="md:w-2/3 p-6 flex flex-col justify-center">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center">
                    <div
                      className={`w-10 h-10 rounded-lg ${person.color} bg-opacity-10 text-${person.color.replace("bg-", "")} flex items-center justify-center mr-4`}
                    >
                      <Calendar
                        size={20}
                        className={person.color.replace("bg-", "text-")}
                      />
                    </div>
                    <div>
        ...
        <div className="bg-teal-50 border border-teal-100 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-teal-800/80 max-w-xl">{t.signupText}</p>
            </div>
          </div>
