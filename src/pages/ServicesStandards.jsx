...
export default function ServicesStandards() {
  ...
  const t = {
    breadcrumb:
      lang === "ru"
        ? "ГЛАВНАЯ / СТАНДАРТЫ ГОСУСЛУГ"
        : "БАСТЫ БЕТ / МЕМЛЕКЕТТІК ҚЫЗМЕТТЕР СТАНДАРТТАРЫ",
    title:
      lang === "ru"
        ? "Стандарты оказания государственных услуг"
        : "Мемлекеттік қызметтерді көрсету стандарттары",
    subtitle:
      lang === "ru"
        ? "Нормативные требования, сроки и порядок оказания услуг"
        : "Нормативтік талаптар, қызмет көрсету мерзімдері мен тәртібі",
    search: lang === "ru" ? "Поиск стандарта..." : "Стандартты іздеу...",
    linkText:
      lang === "ru"
        ? "Смотреть на Adilet.zan.kz"
        : "Adilet.zan.kz сайтында көру",
  };
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid gap-6">
          {filtered.map((item) => (
            <div
              key={item.id}
              className="bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition border border-gray-100 flex flex-col md:flex-row items-start md:items-center gap-6 group"
            >
              {/* Иконка */}
