...
export default function ServicesDoctors() {
  ...
  const departments = [
    ...
    {
      id: "Специализированное",
      ru: "Спец. отделение",
      kz: "Мамандандырылған бөлім",
    },
    {
      id: "Женская консультация",
      ru: "Женская консультация",
      kz: "Әйелдер консультациясы",
    },
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="max-w-md mx-auto relative mb-8">
            ...
            <input
              type="text"
              placeholder={
                lang === "ru" ? "Поиск врача..." : "Дәрігерді іздеу..."
              }
              value={searchTerm}
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          {loading ? (
            ...
            <div className="py-20 text-center text-gray-400">
              ...
              <p>
                {lang === "ru"
                  ? "В этом отделении пока нет данных"
                  : "Бұл бөлімде әзірге деректер жоқ"}
              </p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredSchedule.map((doc, idx) => (
                    ...
                    >
                      ...
                      {[doc.mon, doc.tue, doc.wed, doc.thu, doc.fri].map(
                        (time, i) => (
                          <td
                            key={i}
                            className="p-4 text-center text-[11px] font-bold border-l border-gray-50 tabular-nums"
                          >
                            {time !== "-" ? (
                              <div className="flex flex-col items-center">
                                <Clock
                                  size={12}
                                  className="text-teal-500 mb-1 opacity-50"
                                />
                                {time}
                              </div>
                            ) : (
