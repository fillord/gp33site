...
export default function ServicesHealthSchools() {
  ...
  const t = {
    breadcrumb:
      lang === "ru"
        ? "ГЛАВНАЯ / ШКОЛЫ ЗДОРОВЬЯ"
        : "БАСТЫ БЕТ / ДЕНСАУЛЫҚ МЕКТЕПТЕРІ",
    title:
      lang === "ru"
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-10 border-l-4 border-blue-500 flex items-start">
          ...
          <div>
            <h3 className="text-lg font-bold text-gray-800 mb-2">
              {lang === "ru"
                ? "Зачем посещать школы здоровья?"
                : "Денсаулық мектептеріне неге бару керек?"}
            </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {schools.map((school) => (
            <div
              key={school.id}
              className="bg-white rounded-xl shadow-md hover:shadow-xl transition overflow-hidden border border-gray-100 flex flex-col group"
            >
              {/* Цветная шапка карточки */}
              <div
                className={`${school.color} p-4 flex items-center justify-between text-white`}
              >
