...
export default function ServicesRights() {
  ...
  const t = {
    breadcrumb:
      lang === "ru"
        ? "ГЛАВНАЯ / ПРАВА УСЛУГОПОЛУЧАТЕЛЕЙ"
        : "БАСТЫ БЕТ / ҚЫЗМЕТ АЛУШЫЛАРДЫҢ ҚҰҚЫҚТАРЫ",
    title:
      lang === "ru"
        ? "Права услугополучателей"
        : "Қызмет алушылардың құқықтары",
    subtitle:
      lang === "ru"
        ? "Согласно Закону РК «О государственных услугах»"
        : "«Мемлекеттік көрсетілетін қызметтер туралы» ҚР Заңына сәйкес",
    ...
    helpTitle:
      lang === "ru"
        ? "Ваши права нарушены?"
        : "Сіздің құқықтарыңыз бұзылды ма?",
    helpText:
      lang === "ru"
    ...
    helpBtn:
      lang === "ru" ? "Подать жалобу / Обращение" : "Шағым беру / Өтініш",
  };
    ...
    <div className="flex flex-col min-h-[60vh] bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="max-w-4xl mx-auto text-center mb-12">
          ...
          <p className="text-gray-600 text-lg leading-relaxed">{t.introText}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {t.rights.map((item, index) => (
            <div
              key={index}
              className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-xl transition-shadow border border-gray-100 flex flex-col items-start group"
            >
        ...
        <div className="bg-red-50 border border-red-100 rounded-2xl p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-gray-700 max-w-xl">{t.helpText}</p>
            </div>
          </div>
        ...
        <div className="mt-8 text-center">
          ...
          >
            {lang === "ru"
              ? "Закон РК «О государственных услугах» (Статья 4)"
              : "«Мемлекеттік көрсетілетін қызметтер туралы» ҚР Заңы (4-бап)"}
          </a>
        </div>
      </div>
    </div>
