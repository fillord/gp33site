...
export default function ServicesArea() {
  ...
  const t = {
    breadcrumb:
      lang === "ru"
        ? "ГЛАВНАЯ / ОБСЛУЖИВАЕМАЯ ТЕРРИТОРИЯ"
        : "БАСТЫ БЕТ / ҚЫЗМЕТ КӨРСЕТІЛЕТІН АУМАҚ",
    title:
      lang === "ru"
        ? "Карта обслуживания поликлиники"
        : "Емхананың қызмет көрсету картасы",
    subtitle:
      lang === "ru"
    ...
    stats: [
      ...
      {
        label: { ru: "Район", kz: "Аудан" },
        value: { ru: "Жетысуский", kz: "Жетісу" },
      },
    ],

    streetsTitle:
      lang === "ru" ? "Улицы по участкам" : "Учаскелер бойынша көшелер",
  ...
  const streets = [
    {
      ru: "проспект Райымбека",
      kz: "Райымбек даңғылы",
      houses: "Нечетная сторона (от ул. Кудерина)",
    },
    {
      ru: "проспект Рыскулова",
      kz: "Рысқұлов даңғылы",
      houses: "Четная сторона",
    },
    ...
    {
      ru: "улица Тургута Озала",
      kz: "Тұрғыт Өзал көшесі",
      houses: "Частично (ниже Райымбека)",
    },
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {t.stats.map((stat, i) => (
            <div
              key={i}
              className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-teal-500 flex items-center justify-between"
            >
              <div>
            ...
            <div className="flex-grow overflow-y-auto p-2 space-y-2">
              {filteredStreets.length > 0 ? (
                filteredStreets.map((street, i) => (
                  <div
                    key={i}
                    className="p-3 hover:bg-teal-50 rounded-lg transition group cursor-default border-b border-gray-50 last:border-0"
                  >
                    <div className="flex items-center text-teal-800 font-bold">
                      <MapPin
                        size={16}
                        className="mr-2 text-teal-500 group-hover:text-teal-700"
                      />
                      {street[lang]}
                    </div>
          ...
          <div className="lg:w-2/3 bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 relative group">
            ...
            <img
              ...
              onError={(e) => {
                ...
                e.target.src =
                  "https://via.placeholder.com/800x600?text=Пожалуйста,+добавьте+файл+map.png+в+папку+public";
              }}
            />
            ...
            <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm p-4 rounded-lg shadow-md border border-gray-200 text-xs hidden md:block max-h-[300px] overflow-y-auto">
              <div className="font-bold mb-3 uppercase text-gray-600">
                {t.legendTitle}
              </div>
              <div className="space-y-2">
                {districtColors.map((d) => (
                  <div key={d.id} className="flex items-center">
                    <div
                      className={`w-3 h-3 ${d.color} rounded-full mr-2 border border-gray-300 shadow-sm`}
                    ></div>
            ...
            <div className="absolute bottom-6 left-6 bg-teal-600/90 text-white p-4 rounded-xl shadow-lg backdrop-blur-md pointer-events-none">
              <h4 className="font-bold leading-tight">
                {lang === "ru"
                  ? "Карта участков ГП №33"
                  : "№33 ҚЕ учаскелер картасы"}
              </h4>
            </div>
          </div>
        </div>
