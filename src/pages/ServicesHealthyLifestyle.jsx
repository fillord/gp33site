...
export default function ServicesHealthyLifestyle() {
  ...
  const t = {
    ...
    subtitle:
      lang === "ru"
        ? "5 простых шагов к долголетию и хорошему самочувствию"
        : "Ұзақ өмір сүру мен жақсы көңіл-күйге арналған 5 қарапайым қадам",

    // Калькулятор
    calcTitle:
      lang === "ru"
        ? "Узнайте свой Индекс Массы Тела (ИМТ)"
        : "Дене салмағының индексін (ДСИ) біліңіз",
    ...
    rules: [
      {
        ...
        desc: {
          ru: "Используйте «Метод тарелки»: 1/2 овощи, 1/4 белки, 1/4 углеводы. Ограничьте сахар и соль.",
          kz: "«Тәрелке әдісін» қолданыңыз: 1/2 көкөністер, 1/4 ақуыздар, 1/4 көмірсулар. Қант пен тұзды шектеңіз.",
        },
      },
      {
        icon: Activity,
        ...
        desc: {
          ru: "Минимум 150 минут умеренной активности в неделю. Даже простая ходьба укрепляет сердце.",
          kz: "Аптасына кемінде 150 минут орташа белсенділік. Тіпті қарапайым жаяу жүру жүректі нығайтады.",
        },
      },
      {
        icon: Moon,
        ...
        desc: {
          ru: "Спать нужно 7-8 часов. Ложитесь и вставайте в одно время, проветривайте комнату перед сном.",
          kz: "7-8 сағат ұйықтау керек. Бір уақытта жатып, бір уақытта тұрыңыз, ұйықтар алдында бөлмені желдетіңіз.",
        },
      },
      {
        icon: Smile,
        ...
        desc: {
          ru: "Находите время для хобби и отдыха. Общение с близкими — лучшее лекарство от тревоги.",
          kz: "Хобби мен демалуға уақыт табыңыз. Жақындарыңызбен сөйлесу — мазасыздыққа қарсы ең жақсы дәрі.",
        },
      },
      {
        icon: Heart,
        ...
        desc: {
          ru: "Курение и алкоголь — главные враги сосудов. Бросить никогда не поздно.",
          kz: "Темекі шегу мен алкоголь — тамырлардың басты жаулары. Тастау ешқашан кеш емес.",
        },
      },
    ],
    ...
    schoolLink: {
      title:
        lang === "ru" ? "Хотите узнать больше?" : "Көбірек білгіңіз келе ме?",
      text:
        lang === "ru"
          ? "Посетите наши бесплатные Школы здоровья при поликлинике."
          : "Емхана жанындағы тегін Денсаулық мектептеріне келіңіз.",
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {t.rules.map((rule, idx) => (
            <div
              key={idx}
              className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition border border-gray-100 flex flex-col items-start"
            >
              <div
                className={`${rule.color} w-12 h-12 rounded-full flex items-center justify-center text-white mb-4 shadow-sm`}
              >
                <rule.icon size={24} />
              </div>
          ...
          <div className="bg-teal-600 p-6 rounded-xl shadow-md text-white flex flex-col justify-center items-start">
            ...
            <Link
              to="/patients/schools"
              className="bg-white text-teal-700 font-bold py-2 px-4 rounded-lg hover:bg-teal-50 transition flex items-center text-sm"
            >
          ...
          <div className="md:w-1/2 bg-teal-50 p-8 md:p-12 flex flex-col justify-center items-center text-center border-t md:border-t-0 md:border-l border-gray-100">
            {bmiResult ? (
              <div className="animate-fade-in">
                <div className="text-gray-500 font-bold uppercase tracking-widest mb-2">
                  {t.yourBmi}
                </div>
                ...
                <div
                  className={`text-2xl font-bold ${bmiResult.color} flex items-center justify-center gap-2`}
                >
                  {bmiResult.color === "text-green-500" && (
                    <CheckCircle size={24} />
                  )}
                  {bmiResult.status}
                </div>
              ...
              <div className="opacity-50">
                ...
                <p className="text-gray-500 text-lg">
                  {lang === "ru"
                    ? "Введите рост и вес, чтобы увидеть результат"
                    : "Нәтижені көру үшін бой мен салмақты енгізіңіз"}
                </p>
              </div>
            )}
          </div>
