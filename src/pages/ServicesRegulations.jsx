...
export default function ServicesRegulations() {
  ...
  const t = {
    breadcrumb:
      lang === "ru"
        ? "ГЛАВНАЯ / РЕГЛАМЕНТЫ ГОСУСЛУГ"
        : "БАСТЫ БЕТ / МЕМЛЕКЕТТІК ҚЫЗМЕТТЕР РЕГЛАМЕНТТЕРІ",
    title:
      lang === "ru"
        ? "Регламенты оказания государственных услуг"
        : "Мемлекеттік қызметтерді көрсету регламенттері",
    subtitle:
      lang === "ru"
        ? "Порядок и процедуры оказания услуг населению"
        : "Халыққа қызмет көрсету тәртібі мен рәсімдері",
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid gap-4">
          {filtered.map((item) => (
            <div
              key={item.id}
              className="bg-white p-6 rounded-lg shadow-sm hover:shadow-md transition border-l-4 border-teal-500 flex flex-col md:flex-row items-start md:items-center justify-between gap-4"
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm">{item.desc[lang]}</p>
                </div>
              </div>
