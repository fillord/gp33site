...
export default function ServicesScreening() {
  ...
  const t = {
    breadcrumb:
      lang === "ru" ? "ГЛАВНАЯ / СКРИНИНГИ" : "БАСТЫ БЕТ / СКРИНИНГТЕР",
    title:
      lang === "ru"
        ? "Проверьте свое здоровье бесплатно"
        : "Денсаулығыңызды тегін тексеріңіз",
    subtitle:
      lang === "ru"
    ...
    formTitle:
      lang === "ru" ? "Введите свои данные" : "Деректеріңізді енгізіңіз",
    ageLabel:
      lang === "ru"
        ? "Ваш возраст (полных лет):"
        : "Сіздің жасыңыз (толық жыл):",
    ...
    resultTitle:
      lang === "ru"
        ? "Вам доступны следующие обследования:"
        : "Сізге келесі тексерулер қолжетімді:",
    noResult:
      lang === "ru"
    ...
    note:
      lang === "ru"
        ? "Для прохождения скрининга обратитесь в доврачебный кабинет. При себе иметь удостоверение личности."
        : "Скринингтен өту үшін дәрігерге дейінгі кабинетке хабарласыңыз. Өзіңізбен бірге жеке куәлігіңіз болуы керек.",
  };
  ...
  const checkScreening = () => {
    ...
    if (userAge >= 40 && userAge <= 70) {
      screenings.push({
        id: 1,
        title: {
          ru: "Раннее выявление болезней системы кровообращения и диабета",
          kz: "Қанайналым жүйесі ауруларын және диабетті ерте анықтау",
        },
        desc: {
          ru: "Измерение давления, холестерина, глюкозы крови, ЭКГ.",
          kz: "Қан қысымын, холестеринді, қандағы глюкозаны өлшеу, ЭКГ.",
        },
      });
    }
    ...
    if (userAge >= 40 && userAge <= 70) {
      screenings.push({
        ...
        desc: {
          ru: "Измерение внутриглазного давления.",
          kz: "Көз ішілік қысымды өлшеу.",
        },
      });
    }
    ...
    if (gender === "female" && userAge >= 40 && userAge <= 70) {
      screenings.push({
        id: 3,
        title: {
          ru: "Профилактика рака молочной железы",
          kz: "Сүт безі обырының алдын алу",
        },
        desc: {
          ru: "Маммография (рентген молочных желез).",
          kz: "Маммография (сүт бездерінің рентгені).",
        },
      });
    }
    ...
    if (gender === "female" && userAge >= 30 && userAge <= 70) {
      screenings.push({
        ...
        title: {
          ...
          kz: "Жатыр мойны обырының алдын алу",
        },
        desc: {
          ru: "Цитологическое исследование мазка.",
          kz: "Жағындыны цитологиялық зерттеу.",
        },
      });
    }
    ...
    if (userAge >= 50 && userAge <= 70) {
      screenings.push({
        id: 5,
        title: {
          ru: "Профилактика колоректального рака",
          kz: "Колоректальды обырдың алдын алу",
        },
        desc: {
          ru: "Анализ кала на скрытую кровь.",
          kz: "Жасырын қанға нәжісті талдау.",
        },
      });
    }
    ...
    if (userAge >= 18 && userAge <= 29) {
      screenings.push({
        id: 6,
        title: {
          ru: "Профилактика в Молодежном центре здоровья",
          kz: "Жастар денсаулық орталығындағы профилактика",
        },
        desc: {
          ru: "Консультация психолога, уролога/гинеколога, дерматовенеролога.",
          kz: "Психолог, уролог/гинеколог, дерматовенеролог кеңесі.",
        },
      });
    }
          ...
          <div className="w-full lg:w-2/3">
            {result !== null ? (
              ...
              <div className="bg-white/50 border-2 border-dashed border-gray-300 rounded-xl p-12 text-center h-full flex flex-col items-center justify-center text-gray-400">
                ...
                <p className="text-lg">
                  {lang === "ru"
                    ? "Заполните форму слева, чтобы увидеть результат"
                    : "Нәтижені көру үшін сол жақтағы форманы толтырыңыз"}
                </p>
              </div>
            )}
          </div>
